...
> **v4.2 里程碑**：引擎视角校验——`/view` 经原生 `ctx.skills.list({ cwd })` 查询引擎实际加载的技能集，工作区/会话面板对「已启用但引擎未加载（missing）」「同名被其他来源覆盖（shadowed）」的技能标红色角标（悬停显示原因）；配套 host 纯函数 `engineLoadState`/`collectEngineLoaded`（引擎查询失败降级为不标注，不误报），测试增至 187 条。原生 skills 接口仅作只读校验，磁盘白名单主链路保持不变。
>
