...
// States:
//   'unknown'  — no skills service / query failed: can't judge (no badge)
//   'off'      — not enabled (workspace nor session): no badge
//   'ok'       — enabled AND the engine loads it as project-dsh
//   'shadowed' — enabled, engine loads the SAME name from another layer
//                (preset etc.): enablement is silently overridden
//   'missing'  — enabled but the engine loads no such skill at all
...
function engineLoadState(skill, engineLoaded) {
  ...
  const active = skill.layer === 'workspace' || skill.sessionEnabled === true;
  if (!active) return 'off';
  if (engineLoaded === undefined || engineLoaded === null) return 'unknown';
  const hit = engineLoaded.find((e) => e.name === skill.name);
  if (!hit) return 'missing';
  return hit.source === 'project-dsh' ? 'ok' : 'shadowed';
}
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (is('/view', 'GET')) {
          ...
          // v4.2 engine-visibility: native ctx.skills.list({ cwd }) as the
          // "what the engine actually loads" reference, annotated per skill.
          const [viewData, engineLoaded] = await Promise.all([
            sessionSkillView(sessionId, cwd),
            collectEngineLoaded(ctx.get('skills'), cwd),
          ]);
