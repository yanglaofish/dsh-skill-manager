...
// A cwd argument must be a Windows absolute path (drive letter root).
...
async function assertRegisteredWorkspace(cwd) {
  if (!isAbsolutePath(cwd)) return { ok: false, error: 'invalid cwd: Windows absolute path required' };
  const resolved = resolve(cwd);
  const ws = await listWorkspaces();
  const known = ws.some((w) => w.exists && resolve(w.cwd) === resolved);
  return known
...
async function importSkillZipFromBuffer(buf) {
  ...
  if (entries.length > 1000) return { ok: false, error: 'zip contains too many entries' };
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (is('/import', 'POST')) {
          ...
          for await (const chunk of req) {
            size += chunk.length;
            if (size > 50 * 1024 * 1024) { sendJson(res, 413, { ok: false, error: 'zip too large' }); return; }
            chunks.push(chunk);
          }
