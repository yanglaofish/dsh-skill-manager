  ...
  for (const root of roots) {
    ...
    for (const ent of names) {
      ...
      try {
        ...
        failed.push(`${root}/${base}: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
  }
...
async function unlinkGlobalSkillFromWorkspace(cwd, skillName) {
  const dir = workspaceSkillDir(cwd);
  const base = skillName.endsWith('.md') ? skillName.slice(0, -3) : skillName;
...
async function listSkillFiles(name, cwd) {
  ...
  // Directory form is now the only canonical layout (folder + SKILL.md);
  // single-file skills have no file browser.
...
async function importSkillDocs(items) {
  ...
  for (const item of items.slice(0, 200)) {
    ...
    try {
      // canonical layout: <library>/<name>/SKILL.md
      await mkdir(join(skillsRoot(), skillName), { recursive: true });
      await writeFile(join(skillsRoot(), skillName, 'SKILL.md'), raw, 'utf8');
