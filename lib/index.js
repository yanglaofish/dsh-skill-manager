...
export function apply(ctx) {
  ...
  // Number of preset MODES available (standard/code/minimal/cordis…), from the
  // agent-presets roster. Distinct from presetSkills (skills physically packed
  // into a preset dir — only some presets carry any).
  async function countPresets() {
    try {
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (url.pathname === `${routePath}/list` && req.method === 'GET') {
          const [active, disabled, preset, workspaces, presetCount] = await Promise.all([
            scanDir(skillsRoot()),
  ...
  registerTool({
    ...
    execute: async () => {
      const [active, disabled, preset, presetCount] = await Promise.all([scanDir(skillsRoot()), scanDir(disabledRoot()), scanPresetSkills(), countPresets()]);
      ...
      return {
        ...
        note: `用户层：启用 ${active.length} 个，禁用 ${disabled.length} 个；preset 模式 ${presetCount} 个，其中捆绑技能 ${preset.length} 个。用户目录 ${skillsRoot()}，preset 目录 ${shippedPresetRoot()} / ${userPresetRoot()}`,
      };
    },
  });
