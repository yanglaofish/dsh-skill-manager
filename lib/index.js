...
import { readdir, readFile, writeFile, mkdir, rename, unlink, copyFile, stat, lstat, readlink, symlink, link, rm } from 'node:fs/promises';
...
// Parse raw skill content + assemble a uniform entry object. Shared by
// both the single-file and directory-form readers so they never drift.
...
// Single-file skill: <dir>/<name>.md
async function readSkillEntry(dir, fileName) {
  const filePath = join(dir, fileName);
  const raw = await readFile(filePath, 'utf8');
  return assembleSkillEntry(raw, { filePath, form: 'file', rootDir: dir, fallbackName: basename(fileName, extname(fileName)) });
}
  ...
  try {
    ...
    for (const ent of names) {
      ...
      if (ent.isFile() && ent.name.endsWith('.md')) {
        ...
        // discoverRoot which supports both single-file and directory skills)
        try {
...
async function migrateLegacySkills() {
  ...
  for (const dir of legacy) {
    ...
    for (const ent of names) {
      ...
      const dst = join(lib, ent.name);
      let inLibrary = false;
      try { await stat(dst); inLibrary = true; } catch { /* absent */ }
      if (!inLibrary) {
        try { await copyFile(src, dst); seen.add(ent.name); } catch { continue; }
      }
  ...
  // re-point workspace symlinks that targeted the legacy global root
  try {
    ...
    for (const w of ws) {
      ...
      for (const ent of names) {
        ...
        try {
          const st = await stat(p); // follows symlinks
          ...
          if (ls.isSymbolicLink()) {
            ...
            if (linkTarget === oldTarget) {
              ...
              try { await symlink(join(lib, ent.name), p, 'file'); } catch { /* keep copy */ }
            }
          }
...
// Minimal recursive directory copy (Node's fs.cp may be unavailable on older
// runtimes; this reads/writes files one level at a time).
...
async function scanSkillSources() {
  ...
  for (const w of workspaces) {
    if (!w.exists) continue;
    sources.push({ name: 'project-agents', dir: join(w.cwd, '.agents', 'skills'), adopt: 'move', workspace: w.cwd });
  ...
  for (const it of items) {
    try {
      if (it.form === 'dir' && it.dir) {
        // directory-form skill: copy the whole folder, then remove the source
        ...
        const dst = join(lib, `${it.name}.md`);
        let has = false;
        try { await stat(dst); has = true; } catch { /* absent */ }
        if (!has) {
          await copyFile(it.path, dst);
          imported.push(it.name);
        }
        if (it.adopt === 'move') {
          await unlink(it.path);
          removed.push(it.name);
        }
      }
    } catch {
...
// Uses scanDir so both single-file (.md) and directory-form (<dir>/SKILL.md)
// skills are listed. linked=true means a same-named skill exists in the
...
async function listWorkspaceSkills(cwd) {
  ...
  for (const s of skills) {
    try {
      const globalPath = s.form === 'dir'
        ? join(skillsRoot(), s.name)
        : join(skillsRoot(), s.fileName);
      const gi = await stat(globalPath);
      if (s.form === 'dir' ? gi.isDirectory() : gi.isFile()) {
        s.linked = true;
...
async function linkGlobalSkillToWorkspace(cwd, skillName) {
  ...
  const dir = workspaceSkillDir(cwd);
  // directory-form skill: symlink/copy the whole folder
  if (entry.form === 'dir' && entry.dir) {
  ...
  // single-file skill: symlink/copy the .md
...
// Disable a skill in a workspace by removing its link/copy. Handles both
// directory-form (<dir>/<name>/) and single-file (<dir>/<name>.md).
async function unlinkGlobalSkillFromWorkspace(cwd, skillName) {
  const dir = workspaceSkillDir(cwd);
  // try directory form first: <dir>/<skillName>/
  ...
  } catch { /* not a directory, fall through to file form */ }
  // single-file form: <dir>/<skillName>.md
...
// Look up one skill in the library. Tries the single-file form first
// (<name>.md), then falls back to scanning for a directory form
// (<dir>/SKILL.md whose frontmatter.name matches).
async function findSkill(name) {
  const fileName = name.endsWith('.md') ? name : `${name}.md`;
  try {
    const raw = await readFile(join(skillsRoot(), fileName), 'utf8');
    return await readSkillEntry(skillsRoot(), fileName);
  } catch { /* not a single file, try directory form */ }
  try {
...
async function listSkillFiles(name, cwd) {
  ...
  if (entry.form !== 'dir') return { ok: true, form: 'file', files: [] };
  const tree = await buildFileTree(entry.dir);
  return { ok: true, form: 'dir', dir: entry.dir, files: tree };
}
...
async function readSkillFile(name, relPath, cwd) {
  const entry = await findSkillIn(name, cwd);
  if (!entry) return { ok: false, error: `skill ${name} not found` };
  if (entry.form !== 'dir') return { ok: false, error: 'single-file skill has no extra files' };
...
async function writeSkillFile(name, relPath, content, cwd) {
  ...
  try {
    ...
    return { ok: false, error: `cannot write ${relPath}: ${err instanceof Error ? err.message : String(err)}` };
  }
}
...
async function editSkill(name, { frontmatter, body }) {
  ...
  // rename (move file) only applies to single-file form; directory form keeps
  // its folder and just gets a new frontmatter.name written to SKILL.md
  if (entry.form !== 'dir' && typeof nextFm.name === 'string' && nextFm.name !== entry.name) {
...
// Permanently delete a skill: unlinks the .md (single-file form) or removes
// the whole skill directory (directory form).
  ...
  const unregisterRoute = ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (url.pathname === `${routePath}/skill-files` && req.method === 'GET') {
          const name = url.searchParams.get('name') ?? '';
          const cwd = url.searchParams.get('cwd') ?? '';
        ...
        if (url.pathname === `${routePath}/skill-file` && req.method === 'GET') {
          const name = url.searchParams.get('name') ?? '';
        ...
        if (url.pathname === `${routePath}/skill-file` && req.method === 'POST') {
