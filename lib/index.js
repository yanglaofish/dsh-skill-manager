...
// own config. Resolved from this module's location when possible.
function shippedPresetRoot() {
  return resolve(import.meta.dirname, '..', '..', '..', '..', '..', '..', 'config', 'agent-presets');
}
...
// List enabled workspace skills: every entry under <cwd>/.dsh/skills. Entries
// hard-linked to a global skill are flagged `linked` (single copy); real local
...
async function listWorkspaceSkills(cwd) {
  ...
  try {
    ...
    for (const ent of names) {
      ...
      // hard link detection: same inode as a global skill file
      let linked = false;
      let linkTarget = null;
      try {
        const fi = await stat(filePath);
        const globalFiles = (await readdir(skillsRoot(), { withFileTypes: true }))
          .filter((g) => g.isFile() && g.name.endsWith('.md'));
        for (const g of globalFiles) {
          const gi = await stat(join(skillsRoot(), g.name));
          if (gi.ino === fi.ino && gi.dev === fi.dev) {
            linked = true;
            linkTarget = join(skillsRoot(), g.name);
            break;
          }
        }
      } catch { /* stat failed: treat as local */ }
      let raw;
      try {
...
// Enable a global skill in a workspace by hard-linking it in (same inode:
// single copy, global edits propagate instantly, no admin rights needed).
async function linkGlobalSkillToWorkspace(cwd, skillName) {
  ...
  const linkPath = join(dir, targetName);
  await ensureWorkspaceSkillDir(cwd);
  const target = resolve(entry.filePath);
  try {
    await link(target, linkPath);
  } catch (err) {
    if (err && err.code === 'EEXIST') {
      // already linked: verify it points at the same inode, otherwise refresh
      try {
        const [a, b] = await Promise.all([stat(target), stat(linkPath)]);
        if (a.ino !== b.ino) {
      ...
      return { ok: false, error: `hardlink failed: ${err instanceof Error ? err.message : String(err)}` };
    }
  }
  return { ok: true, name: entry.name, fileName: targetName, linkPath, kind: 'hardlink' };
}
...
// Read one session's skill selection { cwd, workspaceCwd?, enabled: [] }.
...
// Set the session-selected subset. `enabled` is validated against the
// workspace's allowed set (workspace links ∩ global) before persistence.
async function setSessionSkills(sessionId, cwd, enabled) {
  const workspace = await listWorkspaceSkills(cwd);
  const allowed = new Set(workspace.map((s) => s.name));
  const clean = Array.isArray(enabled) ? enabled.filter((n) => allowed.has(n)) : [];
  const cfg = { cwd, enabled: [...new Set(clean)] };
...
async function sessionSkillView(sessionId, cwd) {
  ...
  const cfg = sessionId ? await readSessionConfig(sessionId) : { cwd, enabled: [] };
  const sessionEnabled = new Set(cfg.enabled);
  ...
  return {
    ...
    skills: merged.map((s) => ({
      ...
      layer: s.layer,
      preset: s.preset,
      sessionEnabled: sessionEnabled.has(s.name),
    })),
    session: { id: sessionId, cwd: cfg.cwd, enabled: cfg.enabled },
    workspaceEnabled: workspace.map((w) => w.name),
  };
}
...
export function apply(ctx) {
  ...
  // Resolve a session's working directory from the live session store when
  // the caller did not supply one (browser panels pass only sessionId).
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (url.pathname === `${routePath}/view` && req.method === 'GET') {
          ...
          sendJson(res, 200, { ok: true, ...await sessionSkillView(sessionId, cwd) });
          return;
        }
        ...
        // POST /session/set — { sessionId, cwd?, enabled: [] }
        if (url.pathname === `${routePath}/session/set` && req.method === 'POST') {
          ...
          const result = await setSessionSkills(sessionId, cwd, body.enabled);
          sendJson(res, 200, result);
          return;
        }
  ...
  ctx.tools.register({
    ...
    description: '设置一个会话在指定工作区的技能勾选子集（enabled 数组）。会话只能勾选工作区已启用的技能（交集校验）；会话层只影响该会话使用的技能，不修改全局内容。',
    parameters: {
      sessionId: { type: 'string', required: true, description: '会话 id' },
      cwd: { type: 'string', required: true, description: '工作区绝对路径' },
      enabled: { type: 'array', items: { type: 'string' }, description: '会话勾选的技能名数组（⊆ 工作区启用集）' },
    },
    output: {
      schema: { type: 'object', additionalProperties: true },
      render: (_args, value) => value.ok
        ? [{ type: 'text', text: `会话技能已更新：${value.cfg.enabled.join(', ') || '（空）'}` }]
        : [{ type: 'text', text: value.error ?? 'error' }],
    },
    isConcurrencySafe: () => false,
    execute: async (args) => setSessionSkills(String(args.sessionId ?? ''), String(args.cwd ?? ''), args.enabled),
  });
...
function formatSessionView(value) {
  ...
  return `会话 ${value.session.id ?? '(无)'} 技能视图（cwd=${value.session.cwd || '(未指定)'}）：\n`
    + (lines.join('\n') || '  （无技能）')
    + `\n工作区已启用：${value.workspaceEnabled.join(', ') || '（无）'}`
    + `\n会话勾选：${value.session.enabled.join(', ') || '（无）'}`;
}

export { inject, skillsRoot, disabledRoot, parseSkillDoc, serializeSkillDoc, validSkillFileName, scanDir, readSkillEntry, summarize, findSkill, editSkill, setSkillEnabled, deleteSkill, importSkillZipFromBuffer, formatList, formatWorkspaceList, formatWorkspaceRegistry, formatSessionView, listWorkspaceSkills, linkGlobalSkillToWorkspace, unlinkGlobalSkillFromWorkspace, sessionSkillView, setSessionSkills, readSessionConfig, listWorkspaces, registerWorkspace, renameWorkspace, forgetWorkspace };
