...
export function apply(ctx) {
  ...
  ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        // POST /import — multipart zip upload
        if (url.pathname === `${routePath}/import` && req.method === 'POST') {
          const result = await importSkillZip(req);
          sendJson(res, 200, result);
          return;
        }
  ...
  // ---------- skill lookup across both roots ----------
  ...
  // Import a skill bundle from a zip body: expect SKILL.md at root (or a single
  // top-level folder containing SKILL.md). Extract into the active skills root.
  async function importSkillZip(req) {
    const chunks = [];
    for await (const chunk of req) chunks.push(chunk);
  ...
  ctx.tools.register({
    ...
    execute: async (args) => {
      ...
      try {
        ...
        if (!mdEntry) return { ok: false, error: 'zip 中未找到 SKILL.md' };
        const raw = mdEntry.getData().toString('utf8');
        const { frontmatter } = parseSkillDoc(raw);
        const skillName = typeof frontmatter.name === 'string' ? frontmatter.name : '';
        if (!skillName || !validSkillFileName(`${skillName}.md`)) {
          return { ok: false, error: 'SKILL.md 的 frontmatter 必须声明 kebab-case 的 name' };
        }
        await mkdir(skillsRoot(), { recursive: true });
        const target = join(skillsRoot(), `${skillName}.md`);
        await writeFile(target, raw, 'utf8');
...
export { inject, skillsRoot, disabledRoot };
