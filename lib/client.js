...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SkillManagerPanel(props) {
			...
			return react.createElement("div", { className: "dskm" },
				...
				query.trim()
					...
					? (loading
						...
						: skills.length === 0
							...
							: react.createElement("div", { className: "dskm_layers" },
								...
								skills.map((s) => react.createElement(SkillRow, {
									key: s.name,
									name: s.name,
									desc: s.description || "",
									checked: s.enabled,
									...
									actions: react.createElement("div", { className: "dskm_acts" },
					...
					: react.createElement(WorkspaceSkillsPanel, { hideCwdInput: true, initialSessionId, onStats: setWsStats }),
		...
		function SkillRow({ name, desc, checked, disabled, onToggle, actions }) {
			const handleClick = (e) => {
				if (disabled) return;
		...
		function WorkspaceSkillsPanel(props) {
			...
			const refresh = useCallback(async (targetCwd) => {
				...
				try {
					...
					if (data.ok) {
						setView(data);
						// the view carries the auto-registered workspaces list, so
						// the picker reflects the just-resolved cwd with no
						// separate round-trip (no stale "暂无工作区" race).
						if (Array.isArray(data.workspaces)) setKnown(data.workspaces);
			...
			const loadKnown = useCallback(async () => {
				try {
					...
					if (data.ok) {
						...
						// picker mode: cwd defaults to the most recent live
						// session's workspace (host fallback) — if the host had
						// nothing, fall back to the first registered workspace.
