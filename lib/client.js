...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		".dskm_ftwrap{display:flex;gap:0;min-height:50vh;padding:0;flex-direction:row}" +
		...
		".dskm_ftpath{font-family:ui-monospace,Consolas,monospace;font-size:11px;color:var(--dsw-alias-label-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}" +
		...
		function SkillDetailModal({ skill, cwd, onClose, onSave, saving }) {
			const [mode, setMode] = useState("preview"); // 'preview' | 'edit' | 'files'
			...
			// directory-form skill file tree state
			const isDir = skill?.form === "dir";
			...
			useEffect(() => {
				...
				api(qs).then((data) => {
					if (cancelled) return;
					setFileTree(data.ok ? (data.files ?? []) : []);
				}).catch(() => { if (!cancelled) setFileTree([]); });
				return () => { cancelled = true; };
			}, [isDir, skill?.name, cwd]);
			...
			return react.createElement("div", { className: "dskm_mask",
				...
				react.createElement("div", { className: "dskm_modal " + (isDir ? "dskm_modal_wide" : "") },
					react.createElement("div", { className: "dskm_mhead" },
						react.createElement("span", { className: "dskm_name" }, skill.name || "技能详情"),
						isDir ? react.createElement("span", { className: "dskm_badge" }, "目录形式") : null,
									...
									: fileContent != null
										? react.createElement(react.Fragment, null,
											react.createElement("div", { className: "dskm_ftbar" },
												react.createElement("span", { className: "dskm_ftpath" }, selFile),
												fileEditing
					...
					react.createElement("div", { className: "dskm_mfoot" },
						isDir
							? react.createElement("button", { className: "dskm_btn dskm_btnp", onClick: onClose, disabled: saving || fileSaving }, "关闭")
							: react.createElement(react.Fragment, null,
				...
				return react.createElement("div", { className: "dskm_layers" },
					...
					pg.items.map((r) => {
						...
						return react.createElement(SkillRow, {
							...
							lib: true, // dim the non-button area
							onToggle: null,
