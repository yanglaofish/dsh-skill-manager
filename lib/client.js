...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SkillManagerPanel(props) {
			...
			const [wsSet, setWsSet] = useState([]); // enabled names in the CURRENT workspace (library tab rows)
			const PAGE_SIZE = 10;

			// library tab shows enablement in the CURRENT workspace, so the
			// row's checked state matches what's really enabled and clicking
			// any unchecked row enables it right there.
			useEffect(() => {
				if (!wsCwd) { setWsSet([]); return; }
				let alive = true;
				const params = new URLSearchParams();
				params.set("cwd", wsCwd);
				if (initialSessionId) params.set("sessionId", initialSessionId);
				api("/view?" + params.toString())
					.then((d) => { if (alive && d.ok) setWsSet(Array.isArray(d.workspaceEnabled) ? d.workspaceEnabled : []); })
					.catch(() => {});
				return () => { alive = false; };
			}, [wsCwd, initialSessionId]);
			...
			const toggleSearchSkill = async (r, enable) => {
				...
				try {
					...
					if (data.ok) applyWsChange(r.name, enable);
				} catch (e) {
					setNotice(String(e));
				} finally {
					setBusy(false);
			...
			// keep the library-tab wsSet in sync after any toggle
			const applyWsChange = (name, enable) => {
				setWsSet((prev) => enable
					? [...new Set([...prev, name])]
					: prev.filter((n) => n !== name));
			};

			// enable/disable a library row in the current workspace
			const toggleLibrarySkill = async (name, enable) => {
				if (!wsCwd) {
					setNotice("未选择工作区：请先到「工作区技能」tab 选择目标工作区");
					return;
				}
				...
				try {
					const data = await api("/workspace/toggle", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({ cwd: wsCwd, name, enable }),
					});
					setNotice(data.ok ? (enable ? `已在工作区启用 ${name}` : `已停用 ${name}`) : data.error ?? "操作失败");
					if (data.ok) {
						applyWsChange(name, enable);
						await refresh(); // keeps delete-created skills + stats fresh
					}
				} catch (e) {
					setNotice(String(e));
				} finally {
			...
			// checked = enabled in the CURRENT workspace (same semantics as
			// search results): unchecked rows are the "not enabled yet" ones
			// and clicking them enables them right here.
			const renderGlobalList = () => {
				...
				return react.createElement("div", { className: "dskm_layers" },
					react.createElement("div", { className: "dskm_legend" },
						wsName
							? `勾选 = 已在当前工作区（${wsName}）启用；未勾选的可直接点击启用（库 = 技能池，编辑/删除仍在本页）。`
							: "技能库 = 可用技能池。未定位工作区，无法在此启停；请到「工作区技能」tab 选择工作区。"
					),
					pg.items.map((s) => {
						const isPreset = s.origin === "preset";
						const enabled = isPreset || wsSet.includes(s.name);
						return react.createElement(SkillRow, {
							key: s.name,
							name: s.name,
							desc: s.description || "",
							checked: enabled,
							disabled: isPreset || busy || !wsCwd, // no workspace → nothing to toggle into
							notoggle: isPreset,
							onToggle: (v) => toggleLibrarySkill(s.name, v),
				...
				tab === "workspace"
					...
					: react.createElement("div", { className: "dskm_wsbar dskm_wsrow dskm_libhint" },
						react.createElement("span", { className: "dskm_legend" },
							`勾选 = 在当前工作区（${wsCwd ? baseNameOf(wsCwd) : "未定位"}）启用。操作其他工作区请到「工作区技能」tab 选择。`
						)
					),
