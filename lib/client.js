...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css =
			...
			".dskm_gridwide{grid-template-columns:repeat(auto-fill,minmax(280px,1fr))}" +
			".dskm_card{display:flex;flex-direction:column;gap:5px;min-height:104px;box-sizing:border-box;padding:10px 12px;border:1px solid var(--dsw-alias-border-l1);border-radius:10px;background:var(--dsw-specific-tip);transition:border-color .12s,background .12s}" +
			".dskm_card:hover{border-color:var(--dsw-alias-border-l2)}" +
			...
			".dskm_cardacts{display:flex;gap:6px;margin-top:auto;padding-top:2px}" +
		...
		function SkillManagerPanel(props) {
			...
			const PAGE_SIZE = 24; // grid layout shows far more than single rows
			...
			// One pool card (library or search hit): compact grid card with
			// name + clamped description; preset rows get their tag and stay
			// visually dimmed + unclickable. Action buttons stay bright.
			...
			// search results: grid of SkillCard — same compact look as the library
			// list: pure pool, NO enable/disable and NO snippet line here.
			const renderSearchResults = () => {
				...
				return react.createElement("div", { className: "dskm_ws" },
					...
					react.createElement("div", { className: "dskm_grid dskm_gridwide" }, pg.items.map((r) => poolRow(r, null))),
					renderPager(pg)
				);
			};
			...
			const renderGlobalList = () => {
				...
				return react.createElement("div", { className: "dskm_ws" },
					...
					react.createElement("div", { className: "dskm_grid" }, pg.items.map((s) => poolRow(s, s.origin === "preset"
		...
		function SkillCard({ name, desc, tag, warn, actions }) {
			return react.createElement("div", { className: "dskm_card" },
