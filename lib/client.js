...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css =
			...
			".dskm_skl{flex:none;align-items:center;width:100%;height:49px;margin:8px 0 0;display:flex;position:relative}" +
			".dskm_skbtns{align-items:center;width:100%;display:flex}" +
			".dskm_skbtn{width:100%;height:49px;color:var(--dsw-alias-label-primary);cursor:pointer;background:0 0;border:none;border-radius:12px;align-items:center;gap:8px;padding:0 8px 0 6px;font-family:inherit;font-size:14px;display:inline-flex;overflow:hidden}" +
			".dskm_skbtn:hover{background:var(--dsw-alias-interactive-bg-hover-solid)}" +
			".dskm_skbtn[aria-expanded=true]{background:var(--dsw-alias-interactive-bg-hover)}" +
			".dskm_sklbl{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden;font-size:13px}" +
			".dskm_sklrail{width:36px;height:36px;margin:0}" +
			".dskm_sklrail .dskm_skbtn{border-radius:50%;justify-content:center;width:36px;height:36px;padding:0;font-size:15px}" +
			".dskm_skpanel{z-index:30;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-base);width:430px;max-width:calc(100vw - 24px);max-height:65vh;box-shadow:var(--dsw-shadow-lv2);--dsh-scrollbar-thumb:var(--dsw-alias-scrollbar-bg-l2);--dsh-scrollbar-thumb-hover:var(--dsw-alias-scrollbar-hover-l2);border-radius:12px;flex-direction:column;display:flex;position:fixed;bottom:128px;left:12px;overflow:hidden}" +
			".dskm_skheader{box-sizing:border-box;border-bottom:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);flex:none;justify-content:space-between;align-items:center;min-height:44px;padding:10px 12px;display:flex}" +
			".dskm_title{color:var(--dsw-alias-label-primary);font-size:13px;font-weight:500;line-height:20px}" +
			".dskm_skbody{flex:1;min-height:0;padding:4px 12px 12px;overflow-y:auto}" +
				...
				loading
					...
					: skills.length === 0
						...
						: react.createElement(react.Fragment, null,
							...
							react.createElement("div", { className: "dskm_sect" }, "工作区技能"),
							react.createElement(WorkspaceSkillsPanel, {})
						)
			);
		}
		...
		function WorkspaceSkillsPanel(props) {
			const { initialCwd, initialSessionId, embed } = props;
			...
			const loadKnown = useCallback(async () => {
				try {
					...
					if (data.ok) setKnown(data.workspaces);
				} catch { /* non-fatal */ }
			}, []);
		...
		// ---------- sidebar footer button (workspace skill manager) ----------
		// A dedicated entry beside Settings at the sidebar foot (same seat as the
		// Cordis panel). Clicking toggles a floating panel that manages skills for
		// the current workspace; the host auto-resolves which workspace is
		// current (fallback: most recent live session cwd), so no manual path.
		function SidebarSkillButton(props) {
			const { wide } = props;
			const [open, setOpen] = useState(false);
			return react.createElement("div", {
				className: wide ? "dskm_skl" : "dskm_skl dskm_sklrail",
			},
				open
					? react.createElement("section", {
						className: "dskm_skpanel",
						"data-dskm-panel": true,
						"aria-label": "工作区技能管理",
					},
						react.createElement("header", { className: "dskm_skheader" },
							react.createElement("span", { className: "dskm_title" }, "工作区技能管理")
						),
						react.createElement("div", { className: "dskm_skbody" },
							react.createElement(WorkspaceSkillsPanel, {})
						)
					)
					: null,
				react.createElement("div", { className: "dskm_skbtns" },
					react.createElement("button", {
						type: "button",
						className: "dskm_skbtn",
						"aria-label": "工作区技能管理",
						"aria-expanded": open,
						onClick: () => setOpen((v) => !v),
					},
						"🧠",
						wide ? react.createElement("span", { className: "dskm_sklbl" }, "工作区技能") : null
					)
				)
			);
		}
		...
		exports.SidebarSkillButton = SidebarSkillButton;
		exports.apply = apply;
