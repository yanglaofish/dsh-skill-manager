...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SkillManagerPanel(props) {
			...
			// NO enable/disable here (enablement happens in the workspace tab).
			// Keeps the body snippet so you can locate the right skill.
						...
						: react.createElement("div", { className: "dskm_layers" },
							...
							embed
								? react.createElement("div", { className: "dskm_legend" },
									view.session?.explicit
										? "已固定自选：取消勾选=停用，未勾选的在列表前部。重新勾选全部即回到跟随工作区。"
										: "跟随工作区：勾选=启用，未勾选=停用（未启用的排在列表前面）。"
								)
								: react.createElement("div", { className: "dskm_legend" },
									"勾选 = 在当前工作区启用（未启用的技能排在列表前面，点击即启用）"
								),
							...
							react.createElement("div", { className: "dskm_search" },
								...
								const all = (view.skills ?? [])
									...
									.slice().sort((a, b) => {
										// NOT-enabled first (they are the actionable rows:
										// click to enable), then enabled, presets last
										const rank = (x) => {
											if (x.origin === "preset") return 2;
											const on = embed ? !!x.sessionEnabled : x.layer === "workspace";
											return on ? 1 : 0;
										};
										const ra = rank(a), rb = rank(b);
										if (ra !== rb) return ra - rb;
