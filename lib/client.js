...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css =
			...
			".dskm_mode{flex:none;font-size:11px;line-height:20px;padding:0 10px;border-radius:20px;font-weight:700;white-space:nowrap;letter-spacing:.2px;user-select:none;text-shadow:0 1px 2px rgba(0,0,0,.35)}" +
			".dskm_mode_follow{background:rgba(59,130,246,.32);color:#bfdbfe;border:1px solid rgba(96,165,250,.75)}" +
			".dskm_mode_pin{background:rgba(245,158,11,.38);color:#fde68a;border:1px solid rgba(251,191,36,.8)}" +
		...
		".dskm_btn:disabled{opacity:.6;cursor:not-allowed}" +
			...
			".dskm_btnp{background:transparent;border-color:var(--dsw-alias-accent);color:var(--dsw-alias-accent);font-weight:600}" +
		...
		".dskm_modal_wide{width:min(900px,92vw)}" +
		...
		".dskm_ftwrap{display:flex;gap:0;min-height:50vh;max-height:60vh;padding:0;flex-direction:row}" +
		".dskm_ftleft{flex:0 0 260px;overflow-y:auto;border-right:1px solid var(--dsw-alias-border-l1);padding:8px 4px;max-height:60vh}" +
		...
		".dskm_fview{flex:1;margin:0;padding:10px 14px;overflow:auto;font-family:ui-monospace,Consolas,monospace;font-size:12px;line-height:17px;white-space:pre-wrap;word-break:break-word;background:var(--dsw-specific-input);color:var(--dsw-alias-label-primary);max-height:60vh}" +
		...
		".dskm_fta{flex:1;width:100%;margin:0;padding:10px 14px;border:none;font-family:ui-monospace,Consolas,monospace;font-size:12px;line-height:17px;white-space:pre;overflow-y:auto;background:var(--dsw-specific-input);color:var(--dsw-alias-label-primary);resize:none;outline:none;max-height:55vh}" +
		...
		function SkillDetailModal({ skill, cwd, onClose }) {
			const [mode, setMode] = useState("preview"); // 'preview' | 'files'
			...
			useEffect(() => {
				...
				api(qs).then((data) => {
					if (cancelled) return;
					if (data.ok) setFileTree(data.files ?? []);
					else setFileTreeError(data.error ?? "文件树加载失败");
				}).catch(() => { if (!cancelled) setFileTreeError("文件树加载失败"); });
			...
			return react.createElement("div", { className: "dskm_mask",
				...
				react.createElement("div", { className: "dskm_modal dskm_modal_wide" },
					...
					react.createElement("div", { className: "dskm_tabs" },
						...
						react.createElement("button", { className: "dskm_tab " + (mode === "files" ? "dskm_tab_on" : ""), onClick: () => setMode("files") }, "📁 文件浏览")
					),
					mode === "preview"
					...
					react.createElement("div", { className: "dskm_mfoot" },
						react.createElement("button", { className: "dskm_btn dskm_btnp", onClick: onClose, disabled: fileSaving }, "关闭")
					)
				)
			);
		}
							...
							embed
								? react.createElement("div", { className: "dskm_legend" },
									view.session?.explicit
										? "取消勾选 = 本会话停用；重新勾选全部 = 回到跟随工作区。"
										: "勾选任意技能即切换为独立自选（跟随模式只读展示，改动在工作区 tab 进行）。"
								)
