...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		".dskm_ftnode{display:flex;align-items:center;gap:2px;cursor:pointer;padding:3px 8px;font-size:12px;line-height:18px;color:var(--dsw-alias-label-primary);border-radius:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;user-select:none}" +
		".dskm_caret{flex:none;font-size:9px;opacity:.65;min-width:9px;text-align:center}" +
			...
			if (node.type === "dir") {
				return react.createElement("div", null,
					react.createElement("div", {
						...
						onClick: () => {
							if (depth === 0) return; // root node: always expanded, not collapsible
							setOpen(!open);
						}
					},
