...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SkillManagerPanel(props) {
			...
			// hit-fields underneath; enabled first, then name; paginated
			const renderSearchResults = () => {
				const sorted = (results ?? []).slice().sort((a, b) => {
					const ae = a.origin === "preset" || a.preset || a.enabled ? 1 : 0;
					const be = b.origin === "preset" || b.preset || b.enabled ? 1 : 0;
				...
				return react.createElement("div", { className: "dskm_layers" },
					...
					pg.items.map((r) => {
						...
						return react.createElement(SkillRow, {
							...
							name: r.name,
							desc: r.description || "",
							checked: isPreset ? true : r.enabled,
							disabled: isPreset,
							onToggle: null,
			...
			const renderGlobalList = () => {
				...
				return react.createElement("div", { className: "dskm_layers" },
					react.createElement("div", { className: "dskm_legend" }, "勾选 = 全局启用，未勾选 = 禁用"),
					pg.items.map((s) => {
		...
		function SkillRow({ name, desc, checked, disabled, onToggle, actions, tag, sub }) {
			const handleClick = (e) => {
				if (disabled) return;
						...
						: react.createElement("div", { className: "dskm_layers" },
							...
							embed
								...
								: react.createElement("div", { className: "dskm_legend" }, "勾选 = 在当前工作区启用"),
														(() => {
								const all = (view.skills ?? []).slice().sort((a, b) => {
									const ae = a.origin === "preset" || a.sessionEnabled || a.layer === "workspace" ? 1 : 0;
									const be = b.origin === "preset" || b.sessionEnabled || b.layer === "workspace" ? 1 : 0;
									if (ae !== be) return be - ae;
									return a.name.localeCompare(b.name);
								});
