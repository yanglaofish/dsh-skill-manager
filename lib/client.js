...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function WorkspaceSkillsPanel(props) {
			const { initialCwd, initialSessionId } = props;
			...
			const toggleSession = async (name) => {
				if (!sessionId) return;
				const cur = new Set((view?.session?.enabled ?? []));
				...
				try {
					const data = await api("/session/set", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({ sessionId, cwd, enabled: [...cur] }),
					});
			...
			return react.createElement("div", { className: "dskm_ws" },
				react.createElement("div", { className: "dskm_h" },
					react.createElement("span", { className: "dskm_name" }, "工作区技能"),
				...
				loading
					? react.createElement("div", { className: "dskm_empty" }, "加载中…")
					: !view
						? react.createElement("div", { className: "dskm_legend" }, "输入工作区路径后点「加载」，查看该工作区启用了哪些技能。")
									...
									sessionId && !isPreset && (s.layer === "workspace" || s.enabled)
										? react.createElement("label", { className: "dskm_check" },
											react.createElement("input", {
												...
												checked: inSession,
												disabled: busy,
												onChange: (e) => { e.stopPropagation(); toggleSession(s.name); },
											}),
											"会话"
										)
										: null
		...
		// ---------- conversation dock (session-level skill strip) ----------
		// A collapsible strip above the composer: shows the current session's
		// selected skills and opens the layered editor (workspace links +
		// session subset). The dock entry receives the session standard kit
		// (`useSession`); host resolves the session's cwd automatically.
		function SessionSkillsDock(props) {
			const { useSession } = props;
			const snap = useSession((s) => s); // re-render on any snapshot change
			const sessionId = snap?.sessionId ?? "";
			const [open, setOpen] = useState(false);
			if (!snap || !sessionId) return null;
			return react.createElement("div", { className: "dskm_dock" },
				react.createElement("button", {
					className: "dskm_dockbtn",
					onClick: () => setOpen(!open),
				},
					"🧠 会话技能" + (open ? " ▾" : " ▸")
				),
				open
					? react.createElement(WorkspaceSkillsPanel, { initialSessionId: sessionId })
					: null
			);
		}
		...
		exports.SessionSkillsDock = SessionSkillsDock;
		exports.apply = apply;
