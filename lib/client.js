...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css =
			...
			".dskm_card{display:flex;flex-direction:column;gap:5px;min-height:104px;box-sizing:border-box;padding:10px 12px;border:1px solid var(--dsw-alias-border-l1);border-radius:10px;background:var(--dsw-specific-tip);transition:border-color .12s,background .12s,opacity .12s}" +
			...
			".dskm_tabsrow{display:flex;align-items:center;justify-content:space-between;gap:8px;border-bottom:1px solid var(--dsw-alias-border-l1);margin-bottom:8px}" +
			".dskm_tabsrow .dskm_tabs{border-bottom:none;margin-bottom:0}" +
			".dskm_viewrow{display:flex;align-items:center;gap:8px;padding:2px 0 4px}" +
		...
		function SkillManagerPanel(props) {
			...
			// enable/disable and NO snippet line here.
			const renderSearchResults = () => {
				...
				return react.createElement("div", { className: "dskm_ws" },
					...
					react.createElement("div", { className: poolCls }, pg.items.map((r) => poolRow(r, null))),
					renderPager(pg)
				);
			};
			...
			return react.createElement("div", { className: "dskm" },
				...
				// tab bar + view-mode switch (single rows / 2 cols / 4 cols)
				react.createElement("div", { className: "dskm_tabsrow" },
		...
		function SkillCard({ name, desc, tag, warn, actions, checked, disabled, notoggle, onToggle }) {
			...
			return react.createElement("div", {
				className: "dskm_card" + (checked ? " dskm_card_on" : " dskm_card_off") + (disabled ? " dskm_card_dis" : ""),
				onClick: handleClick,
				role: notoggle ? undefined : "button",
				tabIndex: notoggle ? -1 : 0,
				onKeyDown: (e) => { if ((e.key === " " || e.key === "Enter") && !disabled && !notoggle) { e.preventDefault(); if (onToggle) onToggle(!checked); } },
				...
				loading
					...
					: !view
						...
						: react.createElement("div", { className: "dskm_layers" },
							...
							react.createElement("div", { className: "dskm_viewrow" },
								react.createElement(ViewSwitch, { mode: layout, onChange: (m) => { setLayout(m); setPage(0); } })
							),
														(() => {
